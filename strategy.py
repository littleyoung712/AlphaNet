...
class Factor_Analysis(object):  # change your save_path
    SAVE_PATH = r"result"  # save_path
    BENCHMARK_PATH = "choice_a.xlsx" # benchmark_path
    POOL = 'A Share'
...
if __name__ == '__main__':
    alphanet_prediction = pd.read_csv(r'result\model_prediction1.csv').dropna()  # your file path
    ...
    date_range = Kline.get_trading_days(alphanet_prediction.index[0].strftime('%Y%m%d'),alphanet_prediction.index[-1].strftime('%Y%m%d'))
    ...
    backtester = Factor_Analysis(alphanet_df, 'alphanet', start='20200101', freq='W')  # frequency=5
