...
class Kline(object):  # 返回
    PATH = 'sqlite:///' + r"ashare_price.db"  # path of db storage
    Data = None
    ...
    @staticmethod
    def generate_eod_syntax(codes: Optional[List[str]] = None, dates: Optional[List[str]] = None) -> str:
        if codes:
            if dates:
                syntax = ('SELECT S_INFO_WINDCODE,TRADE_DT,S_DQ_ADJCLOSE,S_DQ_TRADESTATUSCODE,S_DQ_OPEN,S_DQ_CLOSE,S_DQ_HIGH,S_DQ_LOW,'
                          f'S_DQ_AVGPRICE,S_DQ_VOLUME,S_DQ_Amount FROM AShareEODPrices WHERE TRADE_DT in {tuple(dates)} and S_INFO_WINDCODE in {tuple(codes)}')
            else:
                syntax = ('SELECT S_INFO_WINDCODE,TRADE_DT,S_DQ_ADJCLOSE,S_DQ_TRADESTATUSCODE,S_DQ_OPEN,S_DQ_CLOSE,S_DQ_HIGH,S_DQ_LOW,'
                          f'S_DQ_AVGPRICE,S_DQ_VOLUME,S_DQ_Amount FROM AShareEODPrices WHERE S_INFO_WINDCODE in {tuple(codes)}')
        else:
            if dates:
                syntax = ('SELECT S_INFO_WINDCODE,TRADE_DT,S_DQ_ADJCLOSE,S_DQ_TRADESTATUSCODE,S_DQ_OPEN,S_DQ_CLOSE,S_DQ_AVGPRICE,S_DQ_HIGH,S_DQ_LOW, '
                          f'S_DQ_VOLUME,S_DQ_Amount FROM AShareEODPrices WHERE TRADE_DT in {tuple(dates)}')
            else:
                syntax = ('SELECT S_INFO_WINDCODE,TRADE_DT,S_DQ_ADJCLOSE,S_DQ_TRADESTATUSCODE,S_DQ_OPEN,S_DQ_CLOSE,S_DQ_AVGPRICE,S_DQ_HIGH,S_DQ_LOW, '
    ...
    def load_data(self, codes: Optional[List[str]] = None, dates: Optional[List[str]] = None) -> pd.DataFrame:
        ...
        sql_syntax = self.generate_eod_syntax(codes, dates)
        data = pd.read_sql(
            sql_syntax
            , self.PATH)
        data['TRADE_DT'] = data['TRADE_DT'].astype(str)
        data.columns = ['st_code', 'trade_date', 'close_adj', 'trade_state','open', 'close', 'avg_price','high','low','Volume','Amount']
        return data
